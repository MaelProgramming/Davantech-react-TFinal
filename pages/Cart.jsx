...
const Cart = () => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // On utilise increment() pour éviter les problèmes de concurrence (très pro)
      await updateDoc(userRef, {
        balance: increment(-totalPrice) 
      });

      // 2. CRÉATION DE LA COMMANDE DANS 'ORDERS'
      // Firebase crée la collection automatiquement si elle n'existe pas
      await addDoc(collection(db, "orders"), {
        ...
        total: totalPrice,
        createdAt: serverTimestamp(), // Heure du serveur, pas du client
        status: 'completed'
      });

      // 3. ON VIDE LE PANIER (LOCAL)
      clearCart();

      alert("🎉 ¡Compra realizada con éxito! Tu pedido ha sido registrado, tu sueldo ha sido modificado.");
      navigate('/home');

    } catch (error) {
      console.error("Error al procesar compra:", error);
      alert("Hubo un error crítico al conectar con Firestore.");
    } finally {
      setLoading(false);
    }
  };
